...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // 一時ファイルを生成
    $temp_file_path = sys_get_temp_dir() . '/' . uniqid('csv_') . '.csv';
    file_put_contents($temp_file_path, mb_convert_encoding($new_csv_data, "SJIS", "UTF-8"));

    // ダウンロードリンクを出力
    $download_url = 'download.php?file=' . urlencode(basename($temp_file_path));
    echo '<a href="' . $download_url . '">ダウンロードリンク</a>';
}
